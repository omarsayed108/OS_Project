...
#else
	// When USE_KHEAP is not defined, iterate through ptr_pageWorkingSet array
	for (uint32 i = 0; i < cur_env->page_WS_max_size; i++)
	{
		struct WorkingSetElement *ws = &(cur_env->ptr_pageWorkingSet[i]);
		...
		// read PTE permissions correctly
		uint32 perms = pt_get_page_permissions(cur_env->env_page_directory, va);
